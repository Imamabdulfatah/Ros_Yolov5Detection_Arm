...
class YoLov5TRT(object):
    ...
    """
    def __init__(self, file_yaml, PLUGIN_LIBRARY, engine_file_path):
        with open(file_yaml, mode='r', encoding='utf-8') as f:
            file_content = f.read()
        contend = yaml.load(file_content, yaml.FullLoader)
        ctypes.CDLL(PLUGIN_LIBRARY)
        self.CONF_THRESH = contend['CONF_THRESH']
        self.IOU_THRESHOLD = contend['IOU_THRESHOLD']
        # load labels
        self.categories = contend['categories']
        ...
        with open(engine_file_path, "rb") as f: engine = runtime.deserialize_cuda_engine(f.read())
    ...
    def post_process(self, output, origin_h, origin_w):
        ...
        #print(output[1:])
        pred = np.reshape(output[1:], (-1, 38000))[:num, :]
        # Do nms
